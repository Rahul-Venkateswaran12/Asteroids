...
GUILabel::GUILabel(const string& text, FontType font) : mText(text), mFont(font)
{
    if (font == FONT_HELVETICA_18) {
        mFontWidth = 12; // Adjusted for centering
        mFontHeight = 18;
		mGlutFont = GLUT_BITMAP_HELVETICA_18;// Use GLUT font for Helvetica 18
    }
    else {
        mFontWidth = 9;
        mFontHeight = 15;
		mGlutFont = GLUT_BITMAP_9_BY_15;//Use GLUT font for 9x15
    }
}
